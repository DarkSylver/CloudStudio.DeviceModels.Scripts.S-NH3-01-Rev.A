...
env.log(data);
const amonia = data.messages[0][0].measurementValue;
const temperature = data.messages[0][1].measurementValue;
const humidity = data.messages[1][0].measurementValue;

device.endpoints.byAddress("1").updatePpmConcentrationSensorStatus(amonia);
device.endpoints.byAddress("2").updateTemperatureSensorStatus(temperature / 100);
device.endpoints.byAddress("3").updateHumiditySensorStatus(humidity / 100);
}
